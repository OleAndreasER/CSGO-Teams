...
describe("Mainpage", () => {
  ...
  test("Sort works correctly", async () => {
    ...
    fireEvent.change(sortOptionDropdown, { target: { value: "rank-descending" } });
    ...
    fireEvent.change(sortOptionDropdown, { target: { value: "rank-ascending" } });
  })
  ...
  test("Filter to only show favorites works correctly", async () => {
    ...
    expect(await findByText("No teams match your current filters")).toBeInTheDocument();
    fireEvent.click(favoriteCheckbox);
    ...
    let storedFavorites = JSON.parse(localStorage.getItem("isFavorite") ?? "{}");
  ...
  test("Filter to show teams with players from a specific country works correctly", async () => {
    ...
    const selectedCountryDropdown = await findByTestId("selected-country-dropdown");
